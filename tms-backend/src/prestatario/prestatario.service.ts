...
export class PrestatarioService {
  ...
  async createFromUser(
    ...
    const p = this.prestatarioRepo.create({
      ...
      cantidadCamiones: (dto as any).cantidadCamiones ?? null,
      consumoGasolina: (dto as any).consumoGasolina ?? null,
      tiposVehiculos: (dto as any).tiposVehiculos ?? null,
      capacidadPorTipo: (dto as any).capacidadPorTipo ?? null,
      disponible: (dto as any).disponible ?? true,
      disponibilidadDesde: (dto as any).disponibilidadDesde ? new Date((dto as any).disponibilidadDesde) : null,
      disponibilidadHasta: (dto as any).disponibilidadHasta ? new Date((dto as any).disponibilidadHasta) : null,
      zonaCobertura: (dto as any).zonaCobertura ?? null,
    } as any);
  ...
  async update(
    ...
    // perfil del prestatario (para cálculos de oferta)
    if (anyDto.cantidadCamiones !== undefined)
      p.cantidadCamiones = anyDto.cantidadCamiones;
    if (anyDto.consumoGasolina !== undefined)
      p.consumoGasolina = anyDto.consumoGasolina;
    if (anyDto.tiposVehiculos !== undefined)
      p.tiposVehiculos = anyDto.tiposVehiculos;
    if (anyDto.capacidadPorTipo !== undefined)
      p.capacidadPorTipo = anyDto.capacidadPorTipo;
    if (anyDto.disponible !== undefined)
      p.disponible = anyDto.disponible;
    if (anyDto.disponibilidadDesde !== undefined)
      p.disponibilidadDesde = anyDto.disponibilidadDesde ? new Date(anyDto.disponibilidadDesde) : null;
    if (anyDto.disponibilidadHasta !== undefined)
      p.disponibilidadHasta = anyDto.disponibilidadHasta ? new Date(anyDto.disponibilidadHasta) : null;
    if (anyDto.zonaCobertura !== undefined)
      p.zonaCobertura = anyDto.zonaCobertura;
  ...
  /**
   * Obtener perfil completo del prestatario para cálculos de oferta
   * Retorna todos los datos necesarios: cantidad de camiones, consumo de gasolina,
   * tipos de vehículos, capacidad por tipo, disponibilidad y zona de cobertura
   */
  async getPerfil(id: string): Promise<any> {
    const prestatario = await this.findOneById(id);

    // Construir el perfil con todos los datos requeridos
    const perfil = {
      id: prestatario.id,
      name: prestatario.name,
      // Cantidad de camiones
      cantidadCamiones: prestatario.cantidadCamiones ?? null,
      // Consumo de gasolina
      consumoGasolina: prestatario.consumoGasolina ?? null,
      // Tipos de vehículos
      tiposVehiculos: prestatario.tiposVehiculos ?? [],
      // Capacidad de carga por tipo
      capacidadPorTipo: prestatario.capacidadPorTipo ?? null,
      // Disponibilidad
      disponible: prestatario.disponible ?? false,
      disponibilidadDesde: prestatario.disponibilidadDesde ?? null,
      disponibilidadHasta: prestatario.disponibilidadHasta ?? null,
      // Zona de cobertura
      zonaCobertura: prestatario.zonaCobertura ?? null,
      // Datos adicionales existentes que pueden ser útiles
      transportes: prestatario.transportes ?? [],
      maxWeight: prestatario.maxWeight ?? null,
      maxVolume: prestatario.maxVolume ?? null,
      tipoCarga: prestatario.tipoCarga ?? null,
      contenedor: prestatario.contenedor ?? null,
      servicios: prestatario.servicios ?? [],
      rating: prestatario.rating ?? null,
    };
