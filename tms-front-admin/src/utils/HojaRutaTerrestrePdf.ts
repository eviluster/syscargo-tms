// src/utils/HojaRutaPdf.ts
import { jsPDF } from "jspdf";
import templateUrlDefault from "@/assets/hoja-ruta-terrestre.png";
...
/**
 * generateHojaRutaPdf - principal
 *
 * opts.forceRotate: opcional para forzar corrección [0|90|180|270]
 */
export async function generateHojaRutaTerrestrePdf(
  opts: {
    filename?: string;
  ...
  //  const { formData, overlayData } = opts;
  const filename =
    opts.filename || `HojaRuta_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  try {
    ...
    console.error("generateHojaRutaPdf error:", err);
    throw err;
  }
}
