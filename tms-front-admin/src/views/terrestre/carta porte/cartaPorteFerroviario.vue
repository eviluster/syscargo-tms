    ...
    <main class="hoja-ruta-full">
      ...
      >
        <div class="form-header">
          <h1>Carta de Porte — Ferroviaria</h1>
        ...
        <!-- PORTEADOR / DATOS FERROVIARIOS -->
        <section class="card-block">
          <h4 class="block-title">Porteador / Datos ferroviarios</h4>
          <div class="grid-3">
            <div class="field-box">
            ...
            <!-- Aquí los campos ferroviarios nuevos -->
            <div class="field-box">
              <div class="f-label">Vagón Nº</div>
              <div class="f-value">
                <input v-model="form.vagon_no" type="text" />
              </div>
            </div>

            <div class="field-box">
              <div class="f-label">Sellos Nº</div>
              <div class="f-value">
                <input v-model="form.sellos_no" type="text" />
              </div>
            </div>
            ...
            <table class="table bultos-table">
              <thead>
                <tr>
                  ...
                  <th>Marcas / Números / Productos</th>
                  <th style="width: 120px">Valor declarado</th>
                  <th style="width: 120px">Peso bruto (kg)</th>
                  <th style="width: 120px">Volumen</th>
                  <th style="width: 120px">Tarifa unidad</th>
        ...
        <section class="card-block">
          <h4 class="block-title">
            Operación — Distancia / Tiempo / Recorrido
          </h4>
          <div class="grid-op">
            <div class="field-box">
    ...
    <div style="font-family: Arial, Helvetica, sans-serif; color:#000; box-sizing:border-box; padding:12mm;">
      <div style="width:190mm; margin:0 auto; border:1px solid #000; padding:12mm; background:#fff;">
        <header style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
          <h1 style="margin:0;font-size:20px;color:#000;">CARTA DE PORTE (FERROVIARIA)</h1>
        ...
        <section style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px;">
          ...
          <div style="border:1px solid #000;padding:8px;"><div style="font-weight:700;font-size:11px;">Datos Ferroviarios</div><div style="font-size:13px;">Vagón Nº: ${escapeHtml(form.vagon_no)}</div><div style="font-size:13px;">Sellos Nº: ${escapeHtml(form.sellos_no)}</div></div>
        </section>
        ...
        <section style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
          ...
          <div style="border:1px solid #000;padding:8px;">
            ...
            <div>Distancia: ${escapeHtml(String(form.distancia_km))} km — Tiempo: ${escapeHtml(String(form.tiempo_horas))} h</div>
...
async function generatePdfFromForm() {
  ...
  if (errors.length) {
    console.warn("[carta-ferro] Errores de validación:", errors);
    alert("Corrige:\n" + errors.join("\n"));
    return;
  }
  ...
  loading.value = true;
  try {
    console.log("[carta-ferro] Inicio generación PDF");
    console.log(
      "[carta-ferro] snapshot form:",
      JSON.parse(JSON.stringify(form)),
    );

    const printableHtml = buildPrintableHtml();
    console.log("[carta-ferro] HTML generado (chars):", printableHtml.length);
    ...
    console.log("[carta-ferro] iframe temporal agregado al DOM.");
    ...
    console.log("[carta-ferro] content px:", {
      contentWidthPx,
      contentHeightPx,
    });
    ...
    console.log(
      "[carta-ferro] tamaño contenido mm (W x H):",
      widthMm,
      "x",
      heightMm,
    );
    console.log(
      "[carta-ferro] tamaño final página mm con márgenes (W x H):",
      pageWidthMm,
      pageHeightMm,
    );

    const now = new Date();
    const fname = `CartaPorte_Ferro_${form.vagon_no || "sin_vagon"}_${now.toISOString().slice(0, 19).replace(/[:T]/g, "-")}.pdf`;
    ...
    console.log("[carta-ferro] opciones html2pdf:", opt);

    await html2pdf().set(opt).from(iframe.contentWindow!.document.body).save();
    console.log("[carta-ferro] PDF generado:", fname);

    document.body.removeChild(iframe);
    console.log("[carta-ferro] iframe temporal removido.");
    alert("PDF generado: " + fname);
  } catch (err: any) {
    console.error("[carta-ferro] Error generando PDF:", err);
...
<style scoped>
/* Same visual language as Hoja de Ruta / Carta Terrestre */
.page-wrap {
  min-height: 100vh;
  width: 100%;
