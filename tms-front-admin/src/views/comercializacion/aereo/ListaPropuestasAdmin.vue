<template>
  ...
  <!-- Modales (mantengo los tuyos) -->
  <!-- ... (el resto de tus modales se mantienen exactamente igual que antes) ... -->
  ...
  <!-- Reprogram / Cancel / Deliver modals (idénticos al script anterior) -->
  <!-- ... (mantén tus modales existentes) ... -->
</template>
  ...
  setup() {
    ...
    async function confirmAction(p: Proposal, action: "accepted" | "rejected") {
      const confirm = await Swal.fire({
        ...
        text:
          action === "accepted"
            ? "Al aceptar, indicarás que puedes tomar la carga."
            : "Al rechazar, indicarás que no puedes asumir la carga.",
        icon: "question",
        showCancelButton: true,
      ...
      try {
        await performStatusUpdate(p.id, action, msg);
        await Swal.fire({
          icon: "success",
          text:
            action === "accepted"
