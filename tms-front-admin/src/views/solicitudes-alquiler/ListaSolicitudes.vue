                  ...
                  >
                    <div class="ms-2 me-auto">
                      ...
                      <div class="small text-muted mb-1">
                        Rating: {{ p.rating ?? "-" }} · TipoCarga:
                        {{ p.tipoCarga ?? "-" }}
                      </div>

                      <div class="small text-muted">
                        Metros disponibles:
                        {{ p.metros_disponibles_alquiler ?? "-" }} m² · Altura
                        disponible: {{ p.altura_m_alquiler ?? "-" }} m
                      </div>
                      ...
                      <div class="small mt-2">
                        ...
                        <span
                          ...
                          "
                        >
                          {{ p.servicios_prest_alquiler.join(", ") }}
                        </span>
                        <span v-else>-</span>
                      </div>
...
/**
 * INTERFAZ Prestatario mínima para el modal (adaptada a tu entidad nueva)
 */
interface PrestatarioMin {
  ...
  servicios_prest_alquiler?: string[] | null;
  transportes?: any[];
}
...
/* tabla: ahora en la forma que utiliza tu KTDatatable */
const tableHeader = ref([
  ...
  {
    columnName: "Metros (m²)",
    columnLabel: "metros_requeridos",
    sortEnabled: true,
    columnWidth: 110,
  },
  {
    columnName: "Tipo uso",
    columnLabel: "tipo_uso",
    sortEnabled: true,
    columnWidth: 140,
  },
  {
    columnName: "Altura (m)",
    columnLabel: "altura_m",
    sortEnabled: true,
    columnWidth: 100,
  },
  {
    columnName: "Fecha inicio",
    columnLabel: "fecha_inicio",
    sortEnabled: true,
    columnWidth: 120,
  },
  {
    columnName: "Duración",
    columnLabel: "duracion",
    sortEnabled: true,
    columnWidth: 110,
  },
  {
    columnName: "Acceso",
    columnLabel: "acceso_horario",
    sortEnabled: true,
    columnWidth: 130,
  },
  {
    columnName: "Temperatura",
    columnLabel: "control_temperatura",
    sortEnabled: true,
    columnWidth: 130,
  },
  {
  ...
  },
  {
    ...
    columnLabel: "frecuencia",
    sortEnabled: true,
    columnWidth: 130,
  },
  {
    columnName: "Clasificación",
    columnLabel: "clasificacion_mercancia",
    sortEnabled: true,
    columnWidth: 140,
  },
  {
...
/* ---------- helper: normalizar fila para evitar objetos/arrays en celdas ---------- */
function normalizeRow(row: any) {
  ...
  } else if (r.servicios == null || r.servicios === "") {
    r.servicios = "-";
  ...
  // cliente -> string representativo (si es objeto)
  if (r.cliente && typeof r.cliente === "object") {
    r.cliente =
      r.cliente.company ||
      r.cliente.name ||
      r.cliente.user?.username ||
      r.cliente.user?.email ||
      JSON.stringify(r.cliente);
  } else {
    r.cliente = r.cliente ?? "-";
  }

  // assignedPrestatario -> string representativo
  if (r.assignedPrestatario && typeof r.assignedPrestatario === "object") {
    r.assignedPrestatario =
      r.assignedPrestatario.companyName ||
      r.assignedPrestatario.name ||
      r.assignedPrestatario.user?.username ||
      JSON.stringify(r.assignedPrestatario);
  } else {
    r.assignedPrestatario = r.assignedPrestatario ?? "-";
  }

  // fechas -> strings (sobrescribimos)
  r.fecha_inicio = formatDate(r.fecha_inicio ?? null);
  r.created_at = formatDateTime(r.created_at ?? null);

  // comentarios -> truncado
  if (r.comentarios) {
...
const loadSolicitudes = async () => {
  try {
    ...
    // normalizamos filas para evitar que KTDatatable stringify objetos/arrays
...
/* ---------- NUEVAS FUNCIONES: MATCH usando /prestatario/match-space ---------- */

/**
 * Construye params para el nuevo endpoint: metros y altura (numéricos)
 */
function buildPrestatariosParamsFromSolicitud(s: Solicitud) {
  const metros = Number(s.metros_requeridos || 0);
  const altura = Number(s.altura_m || 0);
  return { metros, altura, limit: 50 };
}

/**
 * Abre el modal y solicita prestatarios usando el nuevo endpoint:
 * GET /prestatario/match-space?metros=...&altura=...&limit=...
 */
async function openPrestatariosModal(solicitud: Solicitud) {
  try {
    ...
    console.log("[openPrestatariosModal] params:", params);

    // Llamada al nuevo endpoint
    const res = await api.get("/prestatario/match-space", { params });

    const data = Array.isArray(res.data)
      ? res.data
      : (res.data?.data ?? res.data);
    // garantizamos array plano
...
async function selectPrestatarioForSolicitud(p: any) {
  try {
    ...
    await api.post(`/solicitudes/${s.id}/proposals_alquiler`, {
      prestatarioId: p.id,
...
/* ver perfil (intenta fetch si no trae transportes / info completa) */
  ...
  try {
    ...
    Swal.fire({
      ...
      html: `<div>
        ...
        Metros disponibles: ${profile.metros_disponibles_alquiler ?? "-"} m²<br/>
        Altura disponible: ${profile.altura_m_alquiler ?? "-"} m<br/>
        Servicios: ${Array.isArray(profile.servicios_prest_alquiler) && profile.servicios_prest_alquiler.length ? profile.servicios_prest_alquiler.join(", ") : "-"}
      </div>`,
    });
  } catch (err: any) {
