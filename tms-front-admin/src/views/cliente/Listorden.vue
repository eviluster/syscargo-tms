...
import EditOrdenForm from "../comercializacion/aereo/EditOrdenForm.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    function openEditOrdenModal(orden: any) {
      selectedOrdenForEdit.value = { ...orden };
    ...
    function getRowClass(row: any) {
      switch (row.estado) {
        case "entregado":
          return "delivered-border";
        case "cancelado":
          return "cancelled-border";
        case "reprogramado":
          return "reprogrammed-border";
        default:
          return "";
      }
    }
    ...
    const loadOrdenes = async () => {
      try {
        ...
        if (auth.accessToken) {
          ...
          const data = await api.get("/carga/me");
    ...
    function buildPrestatariosParamsFromOrder(order: any) {
      ...
      const via = order.via;
      return { tipoCarga, weight, volume, via };
    }
    ...
    const onItemSelect = (selectedItems: Array<any> | undefined) => {
      if (!selectedItems) {
        selectedIds.value = [];
        return;
      }

      // Si vienen objetos fila, extraer 'id'; si vienen strings/ids, usarlos tal cual
      if (selectedItems.length > 0 && typeof selectedItems[0] === "object") {
        selectedIds.value = selectedItems.map((it: any) => it?.id ?? it);
      } else {
        selectedIds.value = selectedItems as Array<string>;
      }
    };
    ...
    const gatherSelectedFromDOM = () => {
      try {
        ...
        // para depuración: ver cuántos encontrados
        // eslint-disable-next-line no-console
        // console.log(
        //   "[gatherSelectedFromDOM] found checkedNodes:",
        //   checkedNodes.length,
        // );
        ...
        checkedNodes.forEach((node) => {
          ...
          if (node.closest && node.closest("thead")) {
            // eslint-disable-next-line no-console
            console.log("[gatherSelectedFromDOM] ignore header checkbox");
            return;
          }
          ...
          // si llegamos aquí no pudimos extraer id — opcionalmente loguear para debug
          // eslint-disable-next-line no-console
          console.log(
            "[gatherSelectedFromDOM] could not extract id for node:",
            node,
          );
        });
        ...
        // logs finales para ver qué se recogió
        // eslint-disable-next-line no-console
        // console.log(
        //   "[gatherSelectedFromDOM] -> selectedIds:",
        //   selectedIds.value,
        // );
      } catch (err) {
        // eslint-disable-next-line no-console
        console.error("[gatherSelectedFromDOM] error", err);
      }
    };
    ...
    docHandler = (e: Event) => {
      try {
        ...
        if (isInputCheckbox || isRoleCheckbox || hasAriaChecked) {
          ...
          gatherSelectedFromDOM();
          // eslint-disable-next-line no-console
          console.log("[docHandler] event:", e.type, "target:", target);
        }
      } catch (err) {
        // eslint-disable-next-line no-console
